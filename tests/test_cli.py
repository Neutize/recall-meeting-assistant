...
def test_validate_meeting_url_requires_https_and_allowlisted_host():
    assert cli.validate_meeting_url("https://meet.google.com/example-meeting", ("meet.google.com",))
    with pytest.raises(ValueError):
        cli.validate_meeting_url("http://meet.google.com/example-meeting", ("meet.google.com",))
    with pytest.raises(ValueError):
...
def test_create_bot_command_prints_safe_result_without_network(monkeypatch, capsys):
    ...
    assert cli.main(["create-bot", "https://meet.google.com/example-meeting"]) == 0
